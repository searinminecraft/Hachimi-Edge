use std::{os::raw::c_uint, ptr, sync::{atomic::{self, AtomicBool, AtomicI32, AtomicIsize, AtomicU32, AtomicUsize}, Arc}};
...
use windows::{core::{w, BOOL}, Win32::{
    ...
    UI::{
        ...
        WindowsAndMessaging::{
            CallNextHookEx, DefWindowProcW, EnumWindows, FindWindowW, GetClassNameW, GetClientRect, GetWindowLongPtrW, GetWindowRect, GetWindowThreadProcessId, SetWindowLongPtrW, SetWindowPos, SetWindowsHookExW, UnhookWindowsHookEx,
            GWLP_WNDPROC, GWL_STYLE, HCBT_MINMAX, HHOOK, SIZE_MAXIMIZED, SIZE_MINIMIZED, SW_RESTORE, WINDOW_LONG_PTR_INDEX, SWP_FRAMECHANGED, SWP_NOMOVE, SWP_NOSIZE, SWP_NOZORDER, WH_CBT, WM_INPUT, WM_KEYDOWN, WM_KEYUP, WM_MOUSEMOVE, WM_MOUSEWHEEL, WM_RBUTTONDOWN, WM_RBUTTONUP, WM_SYSKEYDOWN, WM_SYSKEYUP, WNDPROC,
            WM_IME_SETCONTEXT, WM_IME_NOTIFY, WM_ACTIVATE, WA_INACTIVE, WS_MAXIMIZEBOX,
            WM_SIZING, WM_ENTERSIZEMOVE, WM_MOVING, WM_EXITSIZEMOVE, WM_SIZE
        },
    }
}};
...
use super::{gui_impl::input, discord, smtc, taskbar, free_camera};
...
// Set when another tool (e.g. umamusume-localify) replaced the WndProc after
// our hook; the active WndProc was re-hooked inline as a fallback.
...
extern "system" fn wnd_proc(hwnd: HWND, umsg: c_uint, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let game_orig = GAME_WNDPROC_ORIG.load(atomic::Ordering::Acquire);
    let orig_raw = if game_orig != 0 { game_orig } else { WNDPROC_ORIG.load(atomic::Ordering::Acquire) };
    ...
    // Freeform window handling (non-Global clients only, matching upstream).
    ...
    match umsg {
        WM_KEYDOWN | WM_SYSKEYDOWN => {
            ...
            // keys like menu_open_key / hide_ingame_ui_hotkey_bind / free-cam.
...
pub fn init() {
    unsafe {
        ...
        // The Komoe/Taiwan PC client is not a Steam release, so it must be
        // searched by title even when is_steam_release is false.
        if game.is_steam_release || game.region == Region::Taiwan {
        ...
        if Hachimi::instance().game.region != Region::Global {
            if let Ok(umamusume_image) = get_assembly_image(c"umamusume.dll") {
                if let Ok(mono_behaviour_extension) =
                    get_class(umamusume_image, c"Gallop", c"MonoBehaviourExtension")
                {
...
// Ported from kairusds/Hachimi-Edge (freeform window support)
...
// Ported from kairusds/Hachimi-Edge: when the game flips to landscape while
// freeform is active, drop freeform so the window doesn't fight the game.
pub fn close_freeform_window_for_landscape() -> bool {
    if !Hachimi::instance().config.load().windows.freeform_window {
        return false;
    }

    // Only safe to call IL2CPP methods from the main thread.
...
// ── Freeform resize machinery (ported from kairusds/Hachimi-Edge) ──────────
...
unsafe extern "system" fn set_window_long_ptr_w_hook(
    ...
    if hwnd.0 == target_hwnd.0 &&
        ...
        if new_long != 0 && new_long != wnd_proc as *const () as isize {
            WNDPROC_ORIG.store(new_long, atomic::Ordering::Release);
