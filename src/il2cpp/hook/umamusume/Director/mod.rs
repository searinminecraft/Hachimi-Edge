...
#[cfg(target_os = "windows")]
fn force_live_fullscreen(this: *mut Il2CppObject) {
    if this.is_null() {
        return;
    }

    let is_free_cam = free_camera::is_scene_enabled(CameraScene::Live);
    let is_trainer_landscape = is_trainer_live() && Hachimi::instance().config.load().trainer_live_landscape;
    if !is_free_cam && !is_trainer_landscape {
        return;
    }
    ...
    if is_trainer_landscape {
        let full_rect = Rect_t { x: 0.0, y: 0.0, width: 1.0, height: 1.0 };
        let main_camera = get_MainCameraObject(this);
        if !main_camera.is_null() {
            Camera::set_rect_Injected(main_camera, &full_rect);
            Camera::ResetAspect(main_camera);
        }

        let trainer_camera = get__trainerCameraTargetCamera(this);
        if !trainer_camera.is_null() {
            Camera::set_rect_Injected(trainer_camera, &full_rect);
            Camera::ResetAspect(trainer_camera);
        }
    }
}
...
pub fn enforce_live_free_camera_output(director: *mut Il2CppObject) {
    force_live_fullscreen(director);
...
static CURRENT_SONG_ID: std::sync::atomic::AtomicI32 = std::sync::atomic::AtomicI32::new(0);

pub fn is_trainer_live() -> bool {
    let cached = CURRENT_SONG_ID.load(Ordering::Relaxed);
    if cached != 0 {
        return cached == 1156;
    }
    let director = instance();
    ...
    CURRENT_SONG_ID.store(music_id, Ordering::Relaxed);
    music_id == 1156
}
...
extern "C" fn Awake(this: *mut Il2CppObject) {
    ...
    let music_id = GetPlaySongId(this);
    CURRENT_SONG_ID.store(music_id, Ordering::Release);
...
extern "C" fn SetupOrientation(this: *mut Il2CppObject, display_mode: DisplayMode) {
    ...
    if is_trainer || config.windows.freeform_window {
        if let Some((width, height)) = crate::windows::wnd_hook::get_client_size() {
            target_display_mode = if width > height || is_trainer {
