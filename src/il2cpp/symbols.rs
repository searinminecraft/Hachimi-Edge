...
pub fn get_method_addr(class: *mut Il2CppClass, name: &CStr, args_count: i32) -> usize {
    ...
    if let Ok(method) = res {
        ...
        let class_name = unsafe {
            crate::il2cpp::api::il2cpp_class_get_name(class)
                .as_ref()
                .and_then(|p| CStr::from_ptr(p).to_str().ok())
                .unwrap_or("unknown")
        };
        warn!("get_method_addr: {}::{} = NULL", class_name, name.to_str().unwrap());
        0
    }
}
...
pub fn get_runtime_type(asm: &CStr, ns: &CStr, name: &CStr) -> *mut Il2CppObject {
    let image = match get_assembly_image(asm) {
        Ok(img) => img,
        Err(_) => return null_mut(),
    };

    let klass = match get_class(image, ns, name) {
        Ok(c) => c,
        Err(_) => return null_mut(),
    };

    let type_obj = il2cpp_class_get_type(klass);
    if type_obj.is_null() {
        return null_mut();
    }
